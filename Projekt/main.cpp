...
// --- ZMIENNE KALKULATORA KONIUNKCJI ---

// --- ZMIENNE KALKULATORA KONIUNKCJI ---

bool showAlignCalculator = false;



// TÊ MAPÊ PODMIEÑ NA NOW¥ WERSJÊ:

std::map<std::string, bool> planetsForAlignment = {

    // Planety

    {"Mercury", false}, {"Venus", false}, {"Earth", true},

    {"Mars", true}, {"Jupiter", true}, {"Saturn", false},
    ...
    {"Io", false}, {"Europa", false}, {"Ganymede", false}, {"Callisto", false},



    // Ksiê¿yce Saturna

    {"Titan", false}, {"Rhea", false}, {"Iapetus", false}, {"Dione", false}, {"Tethys", false},



    // Ksiê¿yce Urana

    {"Titania", false}, {"Oberon", false}, {"Umbriel", false}, {"Ariel", false}, {"Miranda", false},



    // Ksiê¿yce Neptuna

    {"Triton", false}, {"Proteus", false}, {"Nereid", false}

};



float alignmentTolerance = 15.0f;

double foundAlignmentDay = -1.0;

bool isSearchingAlignment = false;

bool alignmentWasSearched = false; // Czy u¿ytkownik chocia¿ raz klikn¹³ "Szukaj"?
...
// Nowe parametry wyszukiwania

int searchRangeYears = 10000; // Standardowo 10 000 lat

float searchStepDays = 1.0f;  // Standardowo co 1 dzieñ



// --- ZMIENNE DO RYSOWANIA LINII KONIUNKCJI ---

bool showAlignLine = false;         // Czy rysowaæ liniê?

std::vector<glm::vec3> alignPoints; // Punkty linii (pozycje planet)

unsigned int alignVAO = 0, alignVBO = 0; // Bufory OpenGL

double alignDayStamp = 0.0;         // Zapamiêtujemy dzieñ, dla którego wyliczono liniê



// --- DANE OBIEKTU WYBRANEGO ---
...
// Struktura do przechowywania opisów
...
"    // Standardowe MVP. Obracanie zrobimy w C++ (model matrix)\n"
...
// --- ZMIENIONA FUNKCJA PROCESS INPUT (OBS£UGA ESC) ---

void processInput(GLFWwindow* window) {

    // Statyczna zmienna do obs³ugi "wciœniêcia raz"
    ...
    if (escPressed && !escPressedLastFrame) {
        ...
        if (gameStarted) {
            ...
            // Jeœli jesteœmy w menu -> WyjdŸ z programu
...
// Funkcja pomocnicza: Oblicza k¹t planety (Heliocentric Longitude) w danym dniu

float CalculateFuturePlanetAngle(CelestialBody* body, double targetDay) {

    if (body->orbit.orbitalPeriod == 0.0) return 0.0f;



    // Wzór: Pozycja startowa + (Ruch na dzieñ * iloœæ dni)

    double dailyMotion = 360.0 / body->orbit.orbitalPeriod;

    double currentAngle = body->orbit.meanAnomalyEpoch + (dailyMotion * targetDay);
...
// Funkcja sprawdzaj¹ca, czy zestaw k¹tów mieœci siê w "linii" (sektorze)

// toleranceDeg - jak szeroka mo¿e byæ "linia" (np. 15 stopni)

bool AreAnglesAligned(std::vector<float>& angles, float toleranceDeg) {
    ...
    // SprawdŸmy najwiêksz¹ dziurê miêdzy planetami
    ...
    // SprawdŸ dziurê miêdzy ostatni¹ a pierwsz¹ (przejœcie przez 360/0)

    float wrapGap = (angles[0] + 360.0f) - angles.back();

    if (wrapGap > maxGap) maxGap = wrapGap;



    // Jeœli najwiêksza dziura to np. 340 stopni, to znaczy ¿e planety s¹ skupione w (360 - 340) = 20 stopniach.

    float spread = 360.0f - maxGap;



    return spread <= toleranceDeg;
...
int main() {
    ...
    // Zakres 15-25 jest lepszy ni¿ 5-10
   ...
   // --- GENEROWANIE PASA G£ÓWNEGO ---
    ...
    for (int i = 0; i < 100; i++) {
        ...
        // ZMIANA: Nadajemy now¹ nazwê przy tworzeniu
    ...
    for (int i = 0; i < 500; i++) {
        ...
        // FIX: Dodajemy losowe nachylenie osi w³asnej
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // --- FIX: LOGIKA KOLIZJI Z CIA£AMI (Wypychanie kamery) ---

        for (auto body : solarSystem.bodies) {

            float visualRadius = (float)body->radius * UNIFIED_RADIUS_SCALE;

            // Promieñ kolizji nieco wiêkszy ni¿ wizualny
            ...
            if (distToBody < collisionDist) {
                ...
                // Jeœli akurat œledzimy ten obiekt, popraw te¿ followDistance
        ...
        if (focusTarget == nullptr) {
            ...
            // (Opcjonalna logika "magnetycznej" kamery przy wolnym locie - zachowana z orygina³u)
        ...
        // ... (Wczeœniejszy kod z camera.GetViewMatrix i Projection) ...



        // --- OBLICZANIE ZAÆMIENIA (ECLIPSE) ---

        // S³oñce jest w punkcie (0,0,0). Sprawdzamy, czy jakaœ planeta przecina liniê Kamera-S³oñce.

        // ... (Po camera.GetViewMatrix i Projection) ...



        // --- REALISTYCZNA OPTYKA ZAÆMIENIA ---

        float eclipseFactor = 1.0f;



        // 1. Definiujemy wizualny rozmiar S³oñca w stopniach

        // S³oñce na niebie ma ok. 0.5 stopnia, ale w grach czêsto robi siê wiêksze (np. 2-3 stopnie) dla efektu.

        // Dopasuj tê wartoœæ do wielkoœci œwiec¹cej kuli w Twoim shaderze.
        ...
        // Wektor kierunku patrzenia na s³oñce (zak³adamy, ¿e jest w 0,0,0)

        // Jeœli s³oñce jest "nieskoñczenie daleko" w skyboxie, to jest to po prostu kierunek na (0,0,0)
        ...
        for (auto body : solarSystem.bodies) {
            ...
            // Ignorujemy obiekty, które nie mog¹ zas³oniæ s³oñca
            ...
            // Planeta musi byæ przed S³oñcem

            if (distToBody > distToSun) continue;



            // 2. Obliczamy rozmiar k¹towy planety z perspektywy kamery

            // arctg(promieñ / dystans) daje po³owê k¹ta widzenia
            ...
            // 3. Obliczamy k¹t separacji (jak daleko œrodek planety jest od œrodka s³oñca na niebie)

            glm::vec3 bodyDir = glm::normalize(camToBody);

            float dotProd = glm::dot(sunDir, bodyDir);

            // Zabezpieczenie przed b³êdami float (acos wymaga zakresu -1 do 1)
            ...
            // Warunek 1: Czy planeta w ogóle dotyka tarczy s³onecznej?

            // (Suma promieni k¹towych > k¹t separacji)

            if (separationAngle < (sunAngularRadius + bodyAngularRadius)) {



                // Warunek 2: Ca³kowite zaæmienie (TOTAL ECLIPSE)

                // Wystêpuje tylko, gdy planeta jest wizualnie wiêksza od s³oñca

                // I znajduje siê w ca³oœci "przed" nim.
                ...
                if (bodyAngularRadius >= sunAngularRadius &&
                    ...
                    // Jesteœmy w strefie cienia ca³kowitego (Umbra)
                    ...
                    // Warunek 3: Czêœciowe zaæmienie (Partial / Annular)

                    // Planeta zas³ania tylko fragment s³oñca lub jest za ma³a (obr¹czkowe).

                    // Obliczamy jak g³êboko planeta "wesz³a" na tarczê s³oneczn¹.



                    // Normalizujemy odleg³oœæ od krawêdzi styku (1.0) do pe³nego pokrycia (0.0)

                    float startOverlap = sunAngularRadius + bodyAngularRadius; // Pierwszy kontakt

                    float fullOverlap = fabs(bodyAngularRadius - sunAngularRadius); // Pe³ne wpisanie



                    // Jak daleko jesteœmy w procesie nak³adania (0.0 = styk, 1.0 = max pokrycie)

                    float coverage = (startOverlap - separationAngle) / (startOverlap - fullOverlap);



                    // Jeœli planeta jest mniejsza od s³oñca (zaæmienie obr¹czkowe),

                    // nigdy nie mo¿emy zejœæ do 0.0 (zawsze widaæ pierœcieñ ognia).

                    float maxDarkness = 0.0f;

                    if (bodyAngularRadius < sunAngularRadius) {

                        // Stosunek powierzchni tarcz (uproszczony)

                        float areaRatio = (bodyAngularRadius * bodyAngularRadius) / (sunAngularRadius * sunAngularRadius);

                        maxDarkness = 1.0f - areaRatio; // Np. zostanie 10% œwiat³a

                    }



                    // Interpolujemy jasnoœæ
        ...
        // Minimalne œwiat³o ambientowe (œwiat³o gwiazd), ¿eby nie by³o idealnej czerni
        ...
        // --- RYSOWANIE LINII KONIUNKCJI ---

        // Warunek znikania: Jeœli puœcimy pauzê, linia znika.

        if (!isPaused && showAlignLine) {
        ...
        if (showAlignLine && !alignPoints.empty()) {
            ...
            orbitShader.setMat4("model", glm::mat4(1.0f)); // Linia jest ju¿ w koordynatach œwiata



            // Kolor linii: Jaskrawy zielony lub turkusowy, ¿eby by³o widaæ

            orbitShader.setVec4("orbitColor", glm::vec4(0.0f, 1.0f, 1.0f, 0.8f));



            glBindVertexArray(alignVAO);

            // GL_LINE_STRIP ³¹czy punkty po kolei

            glLineWidth(2.0f); // Grubsza linia

            glDrawArrays(GL_LINE_STRIP, 0, (GLsizei)alignPoints.size());

            glLineWidth(1.0f); // Reset gruboœci

        }



        // 3. Planety (FIXED: Rotation Logic & Flattening)
        ...
        for (auto body : solarSystem.bodies) {
            ...
            model = glm::rotate(model, glm::radians((float)body->axialTilt), glm::vec3(0, 0, 1)); // Nachylenie (Tilt)

            model = glm::rotate(model, glm::radians((float)body->currentRotationAngle), glm::vec3(0, 1, 0)); // Obrót (Spin)



            // FIX: USUNIÊTO liniê "model = glm::rotate(model, (float)bodyIndex ...)" 

            // która psu³a tekstury asteroid. Teraz u¿ywamy axialTilt.
            ...
            // FIX: Obs³uga tekstur le¿¹cych "na boku" (Jowisz/Saturn)
            ...
            if (needsTextureCorrection) {
                ...
                // Jeœli obracamy model o 90 stopni ¿eby naprawiæ pasy...

                model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

                // ...to oœ Y planety staje siê osi¹ Z modelu, wiêc sp³aszczamy Z zamiast Y
                ...
                // Standardowe sp³aszczenie biegunowe (oœ Y)
        ...
        // Intensywnoœæ zale¿na od zaæmienia

        glUniform1f(glGetUniformLocation(bloomShaderProgram, "intensity"), bloomIntensity * eclipseFactor);



        // --- BILLBOARDING W C++ (POPRAWIONY) ---

        glm::mat4 modelBloom = glm::mat4(1.0f);

        modelBloom = glm::translate(modelBloom, glm::vec3(0.0f, 0.0f, 0.0f)); // S³oñce jest w (0,0,0)



        // Trik: Pobieramy rotacjê z macierzy widoku (view), ¿eby obiekt zawsze by³ obrócony do kamery.

        // Macierz widoku (View) zawiera obrót œwiata wzglêdem kamery.

        // My chcemy, ¿eby obiekt mia³ taki sam obrót jak kamera, wiêc kopiujemy lewy górny róg 3x3.

        // W ten sposób kwadrat zawsze jest "p³aski" wzglêdem Twojego monitora, niezale¿nie gdzie patrzysz.
        ...
        if (!gameStarted) {
            ...
            // Przeliczanie dni na lata (przyjmujemy 365 dni = 1 rok)
            ...
            // Wyœwietlanie w formacie "Rok: X   Dzien: Y"

            ImGui::Text("Rok: %d   Dzien: %.1f", years, days);

            ImGui::Checkbox("Pauza", &isPaused);

            if (ImGui::Button("Otworz Kalkulator Koniunkcji")) {

                showAlignCalculator = !showAlignCalculator;
            ...
            // FIX: ZWIÊKSZONO ZAKRES DO 10,000,000 aby planety mog³y szybciej kr¹¿yæ
            ...
            // MENU WYBORU OBIEKTU (BEZ ASTEROID)
            ...
            if (ImGui::BeginCombo("Sledz Obiekt", currentFocusName)) {
                ...
                for (auto body : solarSystem.bodies) {
                    ...
                    // --- FILTR DEBRIS (Ukrywamy tylko masowe œmieci) ---

                    // Ukrywamy obiekty pasów, które nazwaliœmy "Obiekt Pasa..." lub "Obiekt Pasu..."
                    ...
                    // Na wypadek gdyby jakieœ asteroidy nadal mia³y star¹ nazwê:

                    if (body->name.find("Asteroid") != std::string::npos) continue;



                    // UWAGA: Usunêliœmy filtr "Comet", wiêc Twoje 3 komety teraz siê pojawi¹!
            ...
            if (ImGui::Button("Namierz##Main")) {
                ...
                // ... reszta pêtli wyszukuj¹cej bez zmian ...
            ...
            if (showAlignCalculator) {

                ImGui::Begin("Kalkulator Ustawienia Planet", &showAlignCalculator);

                ImGui::Text("Wybierz planety do ustawienia w linii:");



                ImGui::Columns(3, "align_cols", false);

                for (auto& pair : planetsForAlignment) {
                ...
                ImGui::SliderFloat("Tolerancja (stopnie)", &alignmentTolerance, 1.0f, 45.0f);
                ...
                ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "PARAMETRY ZAAWANSOWANE:");

                ImGui::SliderInt("Zakres (Lata)", &searchRangeYears, 10000, 1000000);

                ImGui::SliderFloat("Krok (Dni)", &searchStepDays, 1.0f, 10.0f, "%.1f");



                ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 0.4f, 0.4f, 1.0f));

                ImGui::TextWrapped("UWAGA: Zwiekszenie zakresu do miliona lat drastycznie komplikuje obliczenia i moze zamrozic aplikacje na kilka sekund~minut!");
                ...
                if (ImGui::Button("SZUKAJ NASTEPNEJ DATY")) {

                    isSearchingAlignment = true;

                    alignmentWasSearched = true;

                    foundAlignmentDay = -1.0;



                    std::vector<CelestialBody*> targetBodies;

                    for (auto body : solarSystem.bodies) {

                        if (planetsForAlignment.count(body->name) > 0 && planetsForAlignment.at(body->name)) {
                    ...
                    if (targetBodies.size() >= 2) {

                        // Wykorzystanie parametrów z UI

                        double startDay = currentTimeDays + 1.0;

                        double maxDays = startDay + (365.0 * (double)searchRangeYears);

                        double step = (double)searchStepDays;



                        for (double d = startDay; d < maxDays; d += step) {

                            std::vector<float> angles;

                            for (auto b : targetBodies) {

                                angles.push_back(CalculateFuturePlanetAngle(b, d));

                            }



                            if (AreAnglesAligned(angles, alignmentTolerance)) {
                ...
                if (isSearchingAlignment) {

                    ImGui::Text("Szukam... Prosze czekac.");

                }

                else if (foundAlignmentDay > 0.0) {

                    int years = (int)(foundAlignmentDay / 365.0);

                    float days = (float)fmod(foundAlignmentDay, 365.0);

                    ImGui::TextColored(ImVec4(0.5f, 1.0f, 0.5f, 1.0f), "ZNALEZIONO!");

                    ImGui::Text("Rok %d, Dzien %.0f", years, days);



                    if (ImGui::Button("Skocz do tej daty")) {

                        currentTimeDays = foundAlignmentDay;

                        isPaused = true;



                        // 1. Aktualizujemy pozycje planet na ten dzieñ

                        solarSystem.update(currentTimeDays);



                        // 2. Zbieramy wybrane planety do narysowania linii

                        std::vector<CelestialBody*> bodiesToConnect;

                        for (auto body : solarSystem.bodies) {

                            if (planetsForAlignment.count(body->name) > 0 && planetsForAlignment.at(body->name)) {

                                bodiesToConnect.push_back(body);
                        ...
                        // 3. SORTOWANIE (Po dystansie od S³oñca)

                        // Dziêki temu linia idzie od œrodka na zewn¹trz bez zygzaków

                        if (!bodiesToConnect.empty()) {

                            std::sort(bodiesToConnect.begin(), bodiesToConnect.end(),

                                [](CelestialBody* a, CelestialBody* b) {

                                    float distA = glm::length(glm::vec3(a->worldPosition));

                                    float distB = glm::length(glm::vec3(b->worldPosition));
                        ...
                        // 4. Budujemy punkty linii

                        alignPoints.clear();

                        for (auto b : bodiesToConnect) {

                            alignPoints.push_back(glm::vec3(b->worldPosition));

                        }



                        // 5. Wysy³amy punkty do buforów OpenGL

                        if (alignVAO == 0) {

                            glGenVertexArrays(1, &alignVAO);

                            glGenBuffers(1, &alignVBO);
                        ...
                        glBindBuffer(GL_ARRAY_BUFFER, alignVBO);

                        glBufferData(GL_ARRAY_BUFFER, alignPoints.size() * sizeof(glm::vec3), alignPoints.data(), GL_STATIC_DRAW);
                        ...
                        // 6. Aktywujemy rysowanie

                        showAlignLine = true;

                        alignDayStamp = currentTimeDays;

                    }

                }

                else if (!alignmentWasSearched) {

                    // Stan pocz¹tkowy - u¿ytkownik jeszcze nie klikn¹³ przycisku

                    ImGui::Text("Kliknij Szukaj...");

                }

                else {

                    // alignmentWasSearched jest true, ale foundAlignmentDay nadal wynosi -1.0

                    ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.5f, 1.0f), "Nie znaleziono w podanym zakresie.");
